...
import matplotlib
matplotlib.use('Agg')  # Using non-GUI backend for Flask apps
...
def forecast_fuel_usage(df):
    ...
    hw_model = ExponentialSmoothing(ts, trend='add', seasonal='add',
                                    seasonal_periods=7)
    hw_fit = hw_model.fit()
    ...
    prophet_df = df.reset_index().rename(
        columns={'timestamp': 'ds', 'fuel_consumption': 'y'})
    ...
    plt.plot(prophet_forecast[-30:], label='Prophet Forecast', 
